class MenuScene extends Phaser.Scene{
  ...
  create () {
      ...
      if(this.registry.get('Level') == 0){
        ...
        // Add images
        this.physics.add.staticGroup().create(((800-this.findText.width)/2.)+170, this.findText.y, 'key').setScale(1).refreshBody();
      ...
      if(this.registry.get('Level') == 1){
        ...
        this.physics.add.staticGroup().create(((800-this.findText.width)/2.)+470, this.findText.y, 'gas').setScale(1.5).refreshBody();
        // Add and center continue text
        this.pressX = this.add.bitmapText(270, 550, 'font', 'PRESS X TO START', 16);
        this.pressX.setX((800-this.pressX.width)/2.);
      } 
      ...
      if(this.registry.get('Level') == 2){
        ...
        this.physics.add.staticGroup().create(((800-this.findText.width)/2.)+470, this.findText.y, 'coffee').setScale(3.5).refreshBody();
        // Add and center continue text
        this.pressX = this.add.bitmapText(270, 550, 'font', 'PRESS X TO START', 16);
        this.pressX.setX((800-this.pressX.width)/2.);
      }
      ...
      if(this.registry.get('Level') == 8){
        ...
        // Add and center main body text
        if(this.registry.get('PlayTom')){
          this.starringText = this.add.bitmapText(20, 180, 'font', "WELL DONE FOR MAKING IT TO THE BONUS ROUND!\n\nYOU AND LYDIA ARE FINALLY BACK HOME AND YOU\n\nCAN FINALLY GO TO SLEEP.\n\nIF YOU CAN COLLECT MORE DREAMS THAN LYDIA IN\n\nONE MINUTE THEN YOU WIN A PRIZE!", 16, 1);
          this.starringText.setX((800-this.starringText.width)/2.);
        }
        else{
          this.starringText = this.add.bitmapText(20, 180, 'font', "WELL DONE FOR MAKING IT TO THE BONUS ROUND!\n\nYOU AND TOM ARE FINALLY BACK HOME AND YOU\n\nCAN FINALLY GO TO SLEEP.\n\nIF YOU CAN COLLECT MORE DREAMS THAN TOM IN\n\nONE MINUTE THEN YOU WIN A PRIZE!", 16, 1);
          this.starringText.setX((800-this.starringText.width)/2.);
        }
        // Add, center and color yellow the find/collect text
        ...
        this.findText.setTint(0xFFFF00);
        // Add images
        this.physics.add.staticGroup().create(470, this.findText.y, 'dream').setScale(2).refreshBody();
        // Add and center continue text
        this.pressX = this.add.bitmapText(270, 550, 'font', 'PRESS X TO START', 16);
        this.pressX.setX((800-this.pressX.width)/2.);
      }
      ...
      if(this.registry.get('Level') == 9){
        ...
        this.add.image(400, 300, 'menu_bonus');
        if(this.registry.get('PlayTom')){
          this.starringText = this.add.bitmapText(20, 140, 'font', "YOU BEAT LYDIA!", 16, 1);
          this.starringText.setX((800-this.starringText.width)/2.);
        }
        else{
          this.starringText = this.add.bitmapText(20, 140, 'font', "YOU BEAT TOM!", 16, 1);
          this.starringText.setX((800-this.starringText.width)/2.);
        }
        // Add images
        this.physics.add.staticGroup().create(300, 420, 'lydia_single').setScale(2.5).refreshBody();
        this.physics.add.staticGroup().create(520, 410, 'tom_single').setScale(3).refreshBody();
